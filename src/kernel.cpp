...
static bool SelectBlockFromCandidates(std::vector<pair<int64_t, uint256> >& vSortedByTimestamp,
    ...
    *pindexSelected = (const CBlockIndex*)0;
	...
	for (auto const& item : vSortedByTimestamp)
    {
        ...
        if(fModifierV2)
            ...
            hashProof = pindex->IsProofOfStake() ? uint256S("0x0") : pindex->GetBlockHash();
...
bool stakeTargetHit(uint256 hashProofOfStake, int64_t nValueIn, uint256 bnTargetPerCoinDay)
{
	...
	arith_uint256 auCoinDayWeight = UintToArith256(uint256(nValueIn));
	auCoinDayWeight /= 100;

//	uint256 bnCoinDayWeight = ArithToUint256(auCoinDayWeight);

    // Now check if proof-of-stake hash meets target protocol
    return (UintToArith256(hashProofOfStake) < (auCoinDayWeight * UintToArith256(bnTargetPerCoinDay)));
}
...
bool CheckStakeKernelHash(unsigned int nBits, const CBlock blockFrom, const CTransaction txPrev, const COutPoint prevout, 
	unsigned int& nTimeTx, unsigned int nHashDrift, bool fCheck, uint256& hashProofOfStake, bool fPrintProofOfStake)
{
    ...
    bnTargetPerCoinDay = ArithToUint256(arith_uint256().SetCompact(nBits));
    ...
    if (fCheck) 
	{
		...
		bool bHit = stakeTargetHit(hashProofOfStake, nValueIn, bnTargetPerCoinDay);
		if (!bHit && fPrintProofOfStake)
		{
		      LogPrintf("FORENSICS::CheckStakeKernelHash() : nValueIn %f, nTryTime %f, prevout.n %f, prevout.hash %s, nTimeBlockFrom %f, modifier %s at height=%d timestamp=%s for block from height=%d timestamp=%s\n",
				(double)nValueIn/COIN, (double)nTimeTx, (double)prevout.n, prevout.hash.GetHex().c_str(), (double)nTimeBlockFrom,
    ...
    for (i = 0; i < (nHashDrift); i++) //iterate the hashing
    {
		...
		{
            LogPrintf("CheckStakeKernelHash() : nValueIn %f, nTryTime %f, prevout.n %f, prevout.hash %s, nTimeBlockFrom %f, modifier %s at height=%d timestamp=%s for block from height=%d timestamp=%s\n",
				(double)nValueIn/COIN, (double)nTryTime, (double)prevout.n, prevout.hash.GetHex().c_str(), (double)nTimeBlockFrom,
                boost::lexical_cast<std::string>(nStakeModifier).c_str(), nStakeModifierHeight,
                DateTimeStrFormat("%Y-%m-%d %H:%M:%S", nStakeModifierTime).c_str(),
                mapBlockIndex[blockFrom.GetHash()]->nHeight,
                DateTimeStrFormat("%Y-%m-%d %H:%M:%S", blockFrom.GetBlockTime()).c_str());
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake)
{
    ...
    if (!ReadBlockFromDisk(blockprev, pindex->GetBlockPos(), chainparams.GetConsensus()))
        return error("CheckProofOfStake(): INFO: failed to find block");

    unsigned int nTime = block.nTime;
    if (!CheckStakeKernelHash(block.nBits, blockprev, txPrev, txin.prevout, nTime, nStakeHashDrift, true, hashProofOfStake, fDebug))
...
unsigned int GetStakeModifierChecksum(const CBlockIndex* pindex)
{
    ...
    assert(pindex->pprev || pindex->GetBlockHash() == chainActive.Genesis()->GetBlockHash());
