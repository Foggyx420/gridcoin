 ...
 *
 *   // network
 *   CGridcoinNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.gridcoin.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CGridcoinNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CGridcoinNetwork(UniValue objIn)
    {
...
// // can be under: GridcoinNetwork
// //   -- signature requirements : Key1(User)
// class CGridcoinNetworkVariable : public CGovernanceObject
// {
// private:
