...
const char * const BITCOIN_CONF_FILENAME = "gridcoin.conf";
const char * const BITCOIN_PID_FILENAME = "gridcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "gridcoin" is a composite category enabling all Gridcoin-related debug output
            if(ptrCategory->count(string("gridcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GridcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GridcoinCore
    // Mac: ~/Library/Application Support/GridcoinCore
    // Unix: ~/.gridcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GridcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GridcoinCore";
#else
    // Unix
    return pathRet / ".gridcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty gridcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override gridcoin.conf
